  ...
  template: `
    <div class="page">
      ...
      @if (loading()) {
            ...
            <tbody>
              @for (app of filteredApps(); track app.applicationRef) {
                <tr class="app-row" (click)="openDetail(app.applicationRef)">
                  ...
                  <td><span class="status-badge" [class]="'status-' + app.status">{{ app.status }}</span></td>
        ...
        <div class="count-label">Showing {{ filteredApps().length }} of {{ apps().length }} applications</div>
      }
    </div>
  `,
  styles: [`
    ...
    h1 { font-size: 1.4rem; font-weight: 700; color: var(--color-text); margin: 0; }
    ...
    .app-row { cursor: pointer; transition: background 0.1s; }
    ...
    .ref-cell { font-family: monospace; font-weight: 600; font-size: 0.8rem; }
    .amount-cell { font-weight: 600; }
    .date-cell { color: var(--color-text-muted); font-size: 0.82rem; white-space: nowrap; }
    ...
    .count-label { font-size: 0.8rem; color: var(--color-text-muted); margin-top: 0.75rem; }
  ...
  filteredApps(): StaffApplicationSummary[] {
    return this.apps().filter(a => {
      const matchSearch = !this.searchTerm ||
        a.applicationRef.toLowerCase().includes(this.searchTerm.toLowerCase());
