...
/**
 * Repository for the {@link ApplicantProfile} entity.
 *
 * <p>An applicant profile has a one-to-one relationship with {@link LoanApplication}. All queries
 * are scoped by {@code tenantId} to enforce tenant isolation.
 */
@Repository
public interface ApplicantProfileRepository extends JpaRepository<ApplicantProfile, Long> {
   ...
   *
   * <p>Used by the customer-facing controller to restrict "my applications" to the profiles the
   * authenticated customer's client identifier actually owns.
   *
